"""Policy-as-code engine — deterministic rule evaluation over versioned YAML."""
...
@lru_cache
def _load_rules_cached(rules_dir: str) -> tuple[PolicyRule, ...]:
    path = Path(rules_dir)
    if not path.is_dir():
        raise PolicyLoadError(f"Policy rules directory not found: {path}")

    rules: list[PolicyRule] = []
    seen: dict[str, Path] = {}
    for rule_path in sorted(path.glob("*.yaml")):
        try:
            raw = yaml.safe_load(rule_path.read_text(encoding="utf-8")) or {}
        except yaml.YAMLError as exc:
            raise PolicyLoadError(f"{rule_path.name} is not valid YAML: {exc}") from exc
        for item in raw.get("rules", []):
            rule = PolicyRule(**item)
            if rule.operator not in _OPERATORS:
                raise PolicyLoadError(f"{rule_path.name}: rule '{rule.id}' unknown operator '{rule.operator}'")
            if rule.id in seen:
                raise PolicyLoadError(f"Duplicate rule id '{rule.id}' in {rule_path.name} and {seen[rule.id].name}")
            seen[rule.id] = rule_path
            rules.append(rule)
    if not rules:
        raise PolicyLoadError(f"No rules found in {path}")
    return tuple(rules)


def load_rules() -> tuple[PolicyRule, ...]:
    return _load_rules_cached(str(get_settings().rules_dir))
