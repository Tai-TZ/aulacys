...
6. Look at the Control Panel at the bottom. Verify that there are no profile tabs showing other customers (Bé Hoa, Trần Vui, Huyền Trần), only the actions (Chạy Thẩm Định, Seed tự động) and Tier-3 checkbox.
7. Click on any document badge under **Section 3: Hồ sơ tài liệu kèm theo** (e.g. `cccd`, `sao_ke_luong`, `cic` or `purpose_evidence`). Verify that a beautiful glassmorphism popup overlay opens, showing the document's image (e.g. the specific customer's CCCD card) and its OCR extracted metadata.
8. Click "Chạy Thẩm Định (API)" or "Seed tự động". Verify that the graph execution begins, the page renders the full paper contract layout (`DossierPreviewCard`), the wizard progress state updates dynamically (e.g. Step 3 becomes Green/Failed, and Step 4 waits for HITL approval), and the Node timeline trace populates on the right side.
9. Go back using the top link `← Quay lại danh sách bộ hồ sơ`. Verify that the status of the run application has updated dynamically (e.g. Bé Hoa now displays "Chờ xét duyệt" or "Đã giải ngân" depending on your approval actions).
